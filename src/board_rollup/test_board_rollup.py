...
def _item(id_, kind, title, state, parent=None):
    return {'id': id_, 'kind': kind, 'title': title, 'owner': '', 'state': state,
            'parent': parent, 'checks': [], 'needs': []}
...
def test_counts_stories():
    ...
    assert rows == [{'id': 'i1', 'title': 'Intent 1', 'state': 'open',
                      'care': '', 'stories_total': 3, 'stories_done': 2}]
