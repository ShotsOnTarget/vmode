...
def _start():
    ...
    for _ in range(200):
        try:
            socket.create_connection(("127.0.0.1", port), timeout=0.1).close(); return port
        except OSError:
            pass
    raise RuntimeError("board_serve did not start")
def _intent():
    return record_run(['create', 'I', '-t', 'epic', '-l', 'kind:intent,care:high',
                        '-a', 'alice', '--no-inherit-labels'])['id']
def _validation(iid):
    vid = record_run(['create', 'V', '-t', 'task', '-l', 'kind:validation,state:waiting',
                       '-a', 'board', '--no-inherit-labels'])['id']
    record_run(['dep', 'add', vid, iid, '-t', 'validates']); return vid
def _get(url):
...
def test_decide_roundtrip(bd_repo):
    ...
    req = urllib.request.Request(f'http://127.0.0.1:{port}/api/decide', data=body,
