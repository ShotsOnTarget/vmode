...
def test_parent_and_checks(bd_repo):
    ...
    verification = _id(
        record_run(["create", "verification1", "-a", "me", "--no-inherit-labels"])
    )
...
def test_needs_from_blocks(bd_repo):
    ...
    story_a = _id(
        record_run(
            ["create", "storyA", "-a", "me", "--no-inherit-labels", "--parent", intent]
        )
    )
    story_b = _id(
        record_run(
            ["create", "storyB", "-a", "me", "--no-inherit-labels", "--parent", intent]
        )
    )
