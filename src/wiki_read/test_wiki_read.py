...
def _page(pattern="Some pattern text."):
    return {
        'id': 'p1', 'title': 'Title One', 'pattern': pattern,
        'evidence': ['ev1', 'ev2'], 'cost': 3, 'fix': 'Do the fix.',
        'created': '2024-01-01T00:00:00', 'last_used': '2024-01-02T00:00:00',
        'times_used': 5,
    }
...
def _write_raw(root, page):
    ...
    lines = ['---'] + [f"{k}: {json.dumps(page[k])}" for k in keys] + [
