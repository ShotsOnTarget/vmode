...
<style>body{font-family:sans-serif}table{border-collapse:collapse}"""
        """td,th{border:1px solid #ccc;padding:4px 8px}tr.row{cursor:pointer}"""
        """#reason{display:none}</style>
</head><body>
<h1>Intents</h1>
<table id="intents"><thead><tr><th>id</th><th>title</th><th>state</th>"""
        """<th>care</th><th>stories</th></tr></thead><tbody></tbody></table>
<div id="tree"></div>
<div id="decide" style="display:none">
<button id="yes">Yes</button><button id="no">No</button>
<span id="reason"><input id="reasonInput" placeholder="reason">"""
...
function loadIntents(){fetch('/api/intents').then(r=>r.json()).then(list=>{
  document.querySelector('#intents tbody').innerHTML=list.map(it=>
    `<tr class="row" data-id="${it.id}"><td>${it.id}</td><td>${it.title}</td>"""
        """<td>${it.state}</td><td>${it.care}</td><td>${it.stories_done}/${it.stories_total}</td></tr>`
  ).join('');});}
function nodeHtml(node){const kids=node.children&&node.children.length?
  '<ul>'+node.children.map(nodeHtml).join('')+'</ul>':'';
  return `<li><a href="#" data-id="${node.id}">${node.id} """
        """${node.title||''}</a>${kids}</li>`;}
function load(id){curId=id;
  ...
  fetch('/api/tree?id='+encodeURIComponent(id)).then(r=>r.json()).then(t=>{
    const back=t.back&&t.back.length?'<div>'+t.back.map(x=>x.id+':'+x.title)."""
        """join(' > ')+'</div>':'';
