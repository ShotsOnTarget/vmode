...
outputs: items whose column_of(...) == name, excluding, in non-'none'-tier columns, any item with a needs entry whose target state is not done (a test job inherits its parent code job's needs), ordered by id
side effects: none
