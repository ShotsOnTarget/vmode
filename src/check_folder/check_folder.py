...
def _files(path: str) -> list[str]:
    """Every file the folder holds, tracked or not, minus tool caches."""
    if not os.path.isdir(path):
        return []
    return sorted(n for n in os.listdir(path) if n not in ("__pycache__",))
...
def check_folder(folder: str, options: dict) -> list[str]:
    ...
    tree to check, default '.'). The folder's file list is read here and
    handed to both gates, so a third file fails whether or not it changed.
    Returns the failed rule names exactly as
    ...
    options = {**options, "files": _files(os.path.join(repo, src))}
