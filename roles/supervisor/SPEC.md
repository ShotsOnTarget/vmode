...
| Built | Only the named files changed (by git diff, never by report). The commit that lands the change names the job id; a change under src/ or tools/ with no job id is rejected, whoever authored it. Formatter check passes. Linter passes. Shape checker passes: file and function length, one public function, three files, six-line note. Work item id present. |
