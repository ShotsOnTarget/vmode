...
| Tokens counted | 943,990 | 8,541,367 |
| Lines code+test | 305 | 149 |
| Tokens per line | 3,095 | 57,325 |
...
5. The Story did not move to the verify column on either tick: prove_once looked for test jobs as children of the Story, but tests are children of their code jobs in the record; a real bug, reopened.
6. No item leaked into the live record and no person committed code.

Target for 0002-5: 3,826 tokens per line was set on the summary figures. On harness figures the baseline does not exist yet. Decision for the Board: adopt the harness figure as the only cost figure from now on, and let 0002-5 close on the next unattended Story measured against 0001-6's 57,325.
