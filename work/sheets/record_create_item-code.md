...
- **Inputs**: kind: one of intent, story, code, test, verification, validation. title: non-empty. owner: non-empty. parent: an existing id, required unless kind is intent. For the right-side kinds test, verification and validation, parent is the item this one checks.
