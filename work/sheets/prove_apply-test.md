...
- **Cases**, one test function each, exactly these names, nothing more:
  - `test_usage_extra_keys_ignored`: gathered usage {'tokens': 5, 'seconds': 1.0, 'report': 'x', 'cost_usd': 0.2} -> prove_apply succeeds and the log line has tokens 5 and no report key
